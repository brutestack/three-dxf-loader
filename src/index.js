...
export function Viewer(data, parent, width, height, font) {
	var drawer = new Drawer(data, font);
	this.scene = drawer.createScene(data);

	if (parent) {
		createViewer(this.scene, parent, width, height);
	}

	function createViewer(scene, parent, width, height) {
...
/**
 * Drawer class for a dxf object.
 * @param {Object} data - the dxf object
 * @param {Object} font - a font loaded with THREE.FontLoader
 * @constructor
 */
export function Drawer(data, font) {
...
class DxfObjectLoader extends THREE.Loader {
	constructor(manager) {
		super(manager);
	}

	load(dataObject, onLoad, onProgress, onError) {
	}
}
